...
# HERO METRICS - Annual Summary Cards
st.title("📊 Financial Dashboard")
st.markdown("### Year-over-Year Performance")

# Calculate metrics for each complete year
num_complete_years = sim_months // 12

if num_complete_years > 0:
    for year_num in range(1, num_complete_years + 1):
        ...
        annual_revenue = year_data['revenue_earned'].sum()
        annual_therapist_costs = year_data['therapist_costs'].sum()
        annual_total_costs = year_data['total_costs'].sum()
        annual_sessions = year_data['billable_sessions'].sum()
        annual_owner_revenue = year_data['owner_revenue'].sum()
        
        gross_profit = annual_revenue - annual_therapist_costs
        net_profit = annual_revenue - annual_total_costs
        profit_margin = (net_profit / annual_revenue * 100) if annual_revenue > 0 else 0
        avg_revenue_per_session = annual_revenue / annual_sessions if annual_sessions > 0 else 0
        avg_capacity_utilization = year_data['capacity_utilization'].mean()
        
        # Calculate delta from previous year if exists
        if year_num > 1:
            prev_start = (year_num - 2) * 12 + 1
            prev_end = (year_num - 1) * 12
            prev_data = df[(df['month'] >= prev_start) & (df['month'] <= prev_end)]
            prev_revenue = prev_data['revenue_earned'].sum()
            prev_owner = prev_data['owner_revenue'].sum()
            prev_sessions = prev_data['billable_sessions'].sum()
            
            revenue_delta = annual_revenue - prev_revenue
            owner_delta = annual_owner_revenue - prev_owner
            sessions_delta = annual_sessions - prev_sessions
        else:
            revenue_delta = None
            owner_delta = None
            sessions_delta = None
        ...
        with col1:
            st.metric(
                "💰 Annual Revenue",
                f"${annual_revenue:,.0f}",
                delta=f"${revenue_delta:,.0f}" if revenue_delta else None,
                delta_color="normal"
            )
        ...
        with col2:
            st.metric(
                "📊 Gross Profit",
                f"${gross_profit:,.0f}",
                help="Revenue minus therapist pay"
            )
        
        with col3:
            st.metric(
                "💵 Owner Net Income",
                f"${annual_owner_revenue:,.0f}",
                delta=f"${owner_delta:,.0f}" if owner_delta else None,
                delta_color="normal",
                help="Owner's total compensation (clinical + business profit)"
            )
        
        with col4:
            st.metric(
                "📈 Total Sessions",
                f"{annual_sessions:,.0f}",
                delta=f"{sessions_delta:,.0f}" if sessions_delta else None,
                delta_color="normal"
            )
        ...
        with col6:
            st.metric(
                "💵 Avg Session Rate",
                f"${avg_revenue_per_session:.0f}"
            )
        
        with col7:
            st.metric(
                "⚡ Capacity Used",
                f"{avg_capacity_utilization:.0f}%"
            )
        
        with col8:
            end_cash = year_data.iloc[-1]['cash_balance']
            st.metric(
                "💼 Cash Balance",
                f"${end_cash:,.0f}",
                help="End of year"
    ...
    st.info("⏳ Simulate at least 12 months to see annual metrics")
