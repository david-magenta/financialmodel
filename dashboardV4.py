...
if owner_initial_clients > 0:
    therapist_cohorts[0].append(ClientCohort(
        start_month=0,  # Started before simulation
...
for month in range(1, sim_months + 1):
    ...
    # Therapist costs - each therapist paid for their own clients' sessions
    ther_cost_total = 0.0
    ...
    month_data["owner_therapist_pay"] = owner_pay_salary  # Show actual salary, not including tax
...
for yr in range(1, (sim_months // 12) + 2):
    ...
    if start <= sim_months:
        ...
        # Use actual salary, not including payroll tax
